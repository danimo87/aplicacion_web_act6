...
export class PrestamosService {
  ...
  async crearPrestamo(id_usuario: number, id_libro: number) {
    ...
    const nuevoPrestamo = await this.dataSource.query(
      `INSERT INTO tbl_prestamos (id_usuario, id_libro, fecha_devolucion, id_estado_prestamo)
       VALUES ($1, $2, CURRENT_DATE + INTERVAL '7 days', 1)
       RETURNING *`,
      [id_usuario, id_libro]
    );
    ...
    const libroActualizado = await this.dataSource.query(
      `UPDATE tbl_libros SET stock = stock - 1 WHERE id_libro = $1 RETURNING stock`,
      [id_libro]
    );
    ...
    if (libroActualizado[0].stock === 0) {
      await this.dataSource.query(
        `UPDATE tbl_libros SET id_estado_libro = 2 WHERE id_libro = $1`,
        [id_libro]
      );
    }
  ...
  async registrarDevolucion(id_prestamo: number) {
    ...
    const prestamo = await this.dataSource.query(
      `SELECT id_libro, id_estado_prestamo FROM tbl_prestamos WHERE id_prestamo = $1`,
      [id_prestamo]
    );
    ...
    await this.dataSource.query(
      `UPDATE tbl_prestamos 
       SET fecha_devolucion_real = CURRENT_DATE, id_estado_prestamo = 2 
       WHERE id_prestamo = $1`,
      [id_prestamo]
    );
  ...
  async renovarPrestamo(id_prestamo: number) {
    ...
    const reservas = await this.dataSource.query(
      `SELECT id_reserva FROM tbl_reservas WHERE id_libro = $1 AND id_estado_reserva = 1`,
      [id_libro]
    );
    ...
    const prestamoRenovado = await this.dataSource.query(
      `UPDATE tbl_prestamos 
       SET fecha_devolucion = fecha_devolucion + INTERVAL '7 days' 
       WHERE id_prestamo = $1 
       RETURNING *`,
      [id_prestamo]
    );
